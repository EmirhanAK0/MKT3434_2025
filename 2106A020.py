    ...
    def load_dataset(self):
        ...
        try:
            ...
            self.X_train, self.X_test, self.y_train, self.y_test = \
                model_selection.train_test_split(X, data.target, 
                                                test_size=test_size, 
                                                random_state=42)
            #___________________________________________________________
        ...
        try:
            ...
            if file_name:
                ...
                if target_col:
                    ...
                    self.X_train = self.handle_missing_values(pd.DataFrame(self.X_train)).values
                    self.X_test = self.handle_missing_values(pd.DataFrame(self.X_test)).values
                    # Split data
                    test_size = self.split_spin.value()
                    self.X_train, self.X_test, self.y_train, self.y_test = \
    ...
    def train_model(self, name, param_widgets):
        try:
            if name == "Linear Regression":
                ...
                self.show_message(f"Loss ({loss_type}): {loss_value:.4f}")
                ...
                # --- Aynı şekilde regression loss hesaplanır ---
                loss_type = self.loss_combo.currentText()
                if loss_type == "MSE":
                    loss_value = mean_squared_error(self.y_test, y_pred)
                elif loss_type == "MAE":
                ...
                # --- Classification için ayrı loss combo kullan ---
                ...
                if loss_type == "Cross-Entropy":
                    ...
                    # Hinge loss sadece binary için uygundur
                    if len(np.unique(self.y_test)) > 2:
                        self.show_error("Hinge Loss sadece binary sınıflandırma için çalışır.")
                        return
                    loss_value = hinge_loss(self.y_test, y_pred)
                ...
                self.show_message(f"Loss (Cross-Entropy): {loss_value:.4f}")
